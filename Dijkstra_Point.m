...
%Run Dijkstra Algorithm for a Point Robot on a given map
% Dijkstra is (A* without heuristic) better conditioned. It searches for
% all the possible nodes before finding the path.
...
%check if the start/goal point is possible or not
...
[path, nodeExplored] = DijkstraGrid (map, start_point, dest_point);
return
end

function [path,nodeExplored] = DijkstraGrid (input_map, start_point, dest_point)
...
while true
    ...
    % Mark the start and destination point in color
...
%     end
    ...
    % Compare the current node and the minimum f (select the minimum f)
    [min_f, cur_node] = min(f(:));
    
    % Break if the destination node is reached or minimum f is infinite (destination does not exist)
...
if (isinf(f(dest_point_lin)))
    ...
    for k = 2:length(path) - 1        
        ...
        xlabel('X in mm')
        ylabel('Y in mm')
        title('Dijkstra Algorithm for Point Robot')
        grid on;
        axis image;
        axis([1 251 1 151])
        drawnow
